...
fire <- read.csv("Selected_fire_information.csv")

# Load abiotic covariate data
cov <- read.csv("Fire_data.csv") %>%
...
Biotic_cov <- read.csv("ZonalStats_AFG_PFG_SHR_TRE_ByFireArea_2021_2024.csv")
...
```{r abiotic-boxplot}
df_abiotic <- read.csv("Fire_data.csv", check.names = FALSE) %>%
  rename(
    FIRE_ID = Name,
    ppt = `ppt (mm)`,
    tmin = `tmin (degrees C)`,
